...
function strokeFor(kind: EdgeKind): React.SVGProps<SVGPathElement> & { className?: string } {
  switch (kind) {
    case 'routes':
    case 'selects':
      return { stroke: 'url(#g-route)', strokeWidth: 2 };
    case 'calls-runtime':
      // Animated dashed flow conveys 'live runtime calls'. The class hook
      // is bound to the @keyframes in the <style> block below. Forward-
      // looking edge kind (not in source spec §6a); see fixtures/edges.ts.
      return {
        stroke: 'url(#g-route)',
        strokeWidth: 2,
        strokeDasharray: '6 6',
        className: 'runway-edge-flow',
      };
    case 'owns':
...
// Inline keyframes for the calls-runtime edge flow. Defined once at module
// level so we don't re-inject the style on every render.
const FLOW_KEYFRAMES = `
  @keyframes runway-edge-flow {
    from { stroke-dashoffset: 0; }
    to { stroke-dashoffset: -36; }
  }
  .runway-edge-flow {
    animation: runway-edge-flow 1.6s linear infinite;
  }
  @media (prefers-reduced-motion: reduce) {
    .runway-edge-flow { animation: none; }
  }
`;
