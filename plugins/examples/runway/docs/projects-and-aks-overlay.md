...
_application_ view. v1 of this work adds three things to the Headlamp
...
OpenAI credential helper — _without any cloud-provider SDK dependency, ever_.
...
A real topology view of each Project is a v1.0 stretch goal — a
working visual proof exists at `plugins/examples/runway/` (see §6a).

When an in-cluster Prometheus is detected, the plugin reads metrics
from it; otherwise it falls back to a small in-memory ring buffer in
the AI Runway backend (§6 / §10). This dual-source pattern lands in
v1.0 and is what makes GPU telemetry (via DCGM) and TTFT-distribution
histograms real in v1.0 on clusters that have Prometheus.
...
several inference providers, but a real ML application is almost never _one_
...
1. **No application identity.** Operators can't answer "what does the
   _Customer-Support RAG_ app look like, and is it healthy?" without a
...
on-call · GitOps integration · multi-cluster Project views · _any_ cloud
...
2. **Inspektor Gadget is provider-neutral.** IG lives in the **core**
   ...
   overlay only adds AKS-_flavored enrichers_ on top (GPU-SKU fit,
...
_only_ to the Kubernetes API server (through Headlamp's proxy) and to
...
makes the AKS overlay an AKS-_detection-and-K8s-glue_ layer rather
...
- **One `Ready` condition**, not per-check (PodsReady, PVCsBound, …).
  ...
  without a consumer. Details for _why_ something is not Ready live
...
- **Namespaced.** A Project is namespaced and its members are in
  _its own namespace_ only. Cross-namespace membership is explicitly
  out of v1.
...
- **Create wizard** — three TS-defined templates:
  - _Chat / single-model._ `Project` CR + `ModelDeployment` + `Service`,
    all labeled with `airunway.ai/project=<name>`.
  - _Model + Storage_ (the RAG-shaped scaffold). `Project` CR +
  ...
  - _Empty._ Creates the `Project` CR alone.
...
- **"Add to project"** affordance on existing ModelDeployment /
  ...
  - **In a different Project** — button reads "Move to this
    ...
    Project _<other>_. Move it to _<this>_?"; on confirm, the
...
Lives as a **tab on Project detail** — always _of_ a Project. Two
...
| ----------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| App metrics | **Prometheus** (when detected via §7.1 probe `prometheus.in-cluster`) **or** the **AI Runway backend's in-memory ring buffer** (fallback) | Prom: `/api/v1/query_range` through the backend; ring buffer: backend Service via Headlamp K8s proxy; both inherit user's token | Backend missing → "Install or repair AI Runway backend" CTA; panel dark, rest of tab renders |
...
> is the one _required_ source; everything else is optional.
...
- **HuggingFace TEI** (Text Embeddings Inference) — embeddings
  engines are half of RAG and the most common non-token-streaming
  shape on AI Runway clusters; ships in-tree so saturation /
  queue-depth stories are legible without a contributed adapter.
  The `tokensPerSec` canonical metric is reused as "embeds/s" for
  TEI (the unit name on the panel is engine-aware); no new
  `CanonicalMetric` enum entries needed in v1.0.

These three engines exercise the adapter contract end-to-end and
together cover the v1.0 demo / fixture corpus.
...
- **Time-series sparklines:** fixed window of **15 minutes**, polled
  every **15 seconds** (~60 points per series). Source chosen per
  cluster: when the `prometheus.in-cluster` probe (§7.1) detects a
  cluster Prometheus, the panel reads from it via the backend's
  `/api/v1/query_range` client; otherwise the backend keeps an
  in-memory rolling ring buffer per ModelDeployment it knows about.
  No new Prometheus dep imposed on the user (ring-buffer path covers
  clusters without one). Ring-buffer mode shows "Collecting…" for the
  first 15 minutes after a new MD appears and again after a backend
  restart (documented behaviour); Prom mode does not have this gap.
  **HA constraint (ring-buffer mode only):** the in-memory buffer is
  per-replica, so the backend Deployment is pinned to `replicaCount:
1` in the Helm chart **when running in ring-buffer mode**.
  Prom-mode is HA-safe — replicas all query the same Prometheus, so
  `replicaCount > 1` is supported. The chart's default value picks
  ring-buffer mode (single replica) until `prometheus.in-cluster` is
  detected; admins who want HA install a cluster Prometheus first.
  Ring buffer is **kept** as the fallback indefinitely (revised from
  the earlier "delete in v1.1" plan) — it's what makes Prom-optional
  actually work.
- **TTFT distribution histogram (drawer):** rendered in v1.0 **only
  when reading from Prometheus** (uses native `..._bucket` series via
  `histogram_quantile()` / direct bucket reads). Ring-buffer mode
  cannot reconstruct distributions and shows quantile bars (p50/p95/
  p99) instead. v1.1 may add bucket retention to the ring buffer; not
  in v1.0 scope.
...
(non-goal §1). Operators who want to see _why_ something is wobbling
...
> **Visual reference.** A working demo of this view — fictional
> Customer Support Agent project, four lanes, per-node face metrics,
> hover-able edges, deep side drawer — lives at
> `plugins/examples/runway/`. It hardcodes all data and runs without
> a cluster connection; treat it as a conversation piece, not a
> reference implementation. The demo's `forward-looking` README
> section maps to v1.1 work (agents, `calls-runtime` edge — see
> §14.7 and §14.8) and to v1.0-when-detected work (Prometheus,
> DCGM — see §6 / §9). See `plugins/examples/runway/README.md` for
> the metric-source table.
...
- **Node kinds:** ModelDeployments, Deployments, Pods (collapsed by
  ...
  Gateways, PVCs, and Secrets/ConfigMaps _only if referenced_ by a
...
- **Edges by relationship:** `routes-to`, `selects`, `owns`, `mounts`,
  ...
  | _(informational)_ Pod | Service            | `calls-runtime` | **v1.1+**, see §14.8 — runtime HTTP calls; not derivable from K8s state alone |
...
### Data source for node face metrics

Per-node face metrics (TTFT / TPOT / tokens-or-embeds-per-sec
sparklines, inline GPU / KV bars) read from the same `useMetrics`
hook the Observability tab consumes (§6). One source means "the
number on the node face matches the number in the panel" is true
by construction; the topology renderer adds no new data path.
...
_not_ add it to any Project. Membership requires the
...
| _(informational)_ Agent runtimes (langgraph, LlamaIndex, AutoGen, …) | **v1.1+**, see §14.7 — would parallel this table with its own override label `airunway.ai/agent-runtime=<name>` | n/a in v1.0                                                      |
...
### Non-AKS probes registered in v1.0

| Probe                   | Signal                                                                                                              | Source               | Used for                                                                                                                            |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------- | -------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `prometheus.in-cluster` | `prometheus-operator` CRDs (`prometheuses.monitoring.coreos.com`) **or** well-known kube-prometheus-stack `Service` | crd + service (high) | Switches §6 app-metrics from ring-buffer-mode to Prom-source mode; enables DCGM rendering (§9) and TTFT-distribution histogram (§6) |

### Reserved probe / label keys (forward-compat for v1.1)

To keep the v1.1 agents-as-first-class work (§14.7) a non-CRD-bump,
v1.0 reserves but does not implement:

- Label keys `airunway.ai/agent-runtime=<name>` and
  `airunway.ai/role=agent` — the §13 webhook must not reject Project
  selectors that reference these keys.
- §7.1 probe name `agent-runtime` — not registered in v1.0; reserved
  so a v1.1 probe can take the name without breaking probe-name
  consumers.

Same pattern as the §5 `Project.spec.references` reservation.
...
- **Rate limiting (two layers, v1.0 numbers hardcoded).**
  - _Plugin-side (UX hint)._ The Diagnostics panel allows at most
    **2 concurrent runs per browser tab**; a third Run button is
    ...
    the backend.
  - _Backend-side (hard cap)._ The AI Runway backend proxies all
  ...
  - _IG-side._ `GadgetManager.RunGadget` exposes no rate-limit
...
- **Rendering (`GadgetRunner.tsx`):** generic table from the gadget's
  column schema (from `GetGadgetInfo`); optional _enrichers_ for
...
2. **GPU coverage — RESOLVED.** IG ships `profile_cuda` (CUDA Driver-
   ...
   **`top_gpu` does not exist** — drop from any curated set or UI copy.
   ...
   framing. The v1.0 panel positions IG as the _primary_ AI-
...
the overlay is an AKS-_detection-and-K8s-glue_ layer.
...
| `aks.dcgm-exporter`     | DCGM exporter `Service` / pod-label `app=nvidia-dcgm-exporter` | service + label (high)                 | enable GPU telemetry panel (renders **only when** `prometheus.in-cluster` also detected; placeholder CTA otherwise) |
...
6. Azure OpenAI **credential helper**: writes a `Secret`
   ...
   **Naming & re-run semantics.** Secret name is user-supplied with
   ...
   - target name exists _and_ carries the wizard annotation →
     confirm-overwrite dialog, then patch.
   - target name exists _without_ the wizard annotation → wizard
     refuses with "A Secret named _X_ exists but wasn't created by
...
- Each step renders as: 1-line description, a code block with the
  exact `az` command pre-filled with detected values _from K8s_
...
- **Wizard-run tracking is server-side via annotation.** Every
  ...
  queries by that annotation and lists what _would_ be deleted,
...
_decorate_ the core IG panel; they don't replace it.

### GPU telemetry panel (DCGM)

When both `aks.dcgm-exporter` and `prometheus.in-cluster` are
detected, the ModelDeployment detail page (and topology node face,
when topology ships) renders a GPU telemetry block sourced from
DCGM metrics via Prometheus: util %, SM occupancy, memory bandwidth,
power, temperature, and (for multi-GPU pods) NCCL bandwidth. When
DCGM is detected but Prometheus is not, the panel shows a "DCGM
exporter detected — install or expose Prometheus to render GPU
telemetry" CTA. When DCGM is not detected, the panel is absent
entirely (no CTA). This is the v1.0 path; hardware-counter metrics
are still outside IG's model (§8) — DCGM-via-Prom is how they get
to the panel.
...
| Phase             | Scope                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **v0.9 (prereq)** | UX-spec items that block Project work (per §0): hard blockers P0-0 (per-cluster backend config) and P0-1 (deployments-list crash + namespace filter); soft prereqs P0-2 (single Settings location) and P0-3 (`useAutoRefresh` hook) land here or alongside early v1.0 steps. Step 1 of v1.0 cannot start until P0-0 and P0-1 are merged.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **v1.0**          | Project CRD + controller (single-label membership + status aggregation); Projects list/detail-as-Resources/wizard + "Add to project"; **Observability tab — app-metrics panel (ring-buffer fallback + Prometheus-as-source when `prometheus.in-cluster` detected) + IG diagnostics panel (gRPC)** (MVP); framework detection (registry + override label, plus `prometheus.in-cluster` and `aks.dcgm-exporter` probes; agent-runtime keys reserved per §7.1); engine adapters for **vLLM, Ray Serve, and HuggingFace TEI**; AKS overlay: `useAksContext`, Azure tab, KAITO preset step, WI SA / ACR / HTTPRoute / AOAI-credential-helper glue, AKS IG enrichers; **GPU telemetry panel via DCGM-over-Prom when both probes detected** (placeholder CTA when only one); **TTFT-distribution histogram in drawer when reading from Prometheus** (quantile bars in ring-buffer-mode); graceful-degradation CTAs everywhere; IG smoke CI (§11). **Stretch:** Topology tab as the default Project view (§6a) — visual reference at `plugins/examples/runway/`. |
| **v1.1**          | Agents as a first-class concept (§14.7) — agent-runtime detection table, `airunway.ai/agent-runtime` override label activated, `role=agent` lane in topology, `AgentRuntimeAdapter` registry; `calls-runtime` edge kind (§14.8) — declarative-first via Agent CRD field / env-var convention, IG-observed fallback; **durable IG runs via `GadgetInstanceManager`** (5-min+ tier, with named-instance ownership/cleanup + a "Running gadgets" list in the Project Observability tab); Project rename; cascade-delete finalizer; multi-Project membership (mechanism TBD based on v1.0 user feedback); more curated gadgets + enrichers from feedback; per-gadget IG version degradation; more KAITO presets; AMD GPU in IG panel; full AKS E2E in CI; if v1.0 deferred topology, ship it here. Optional v1.1 polish: TTFT-distribution histogram in ring-buffer-mode (extends ring buffer to retain bucket counts).                                                                                                                                      |
| **v2.0**          | Azure-Monitor-as-Prometheus historical metrics (still K8s-cluster-local — no cloud SDK); gadget-result persistence; cross-Project ops view; vector-DB / training-shaped Projects.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| #   | Step                                                                                                                                                         |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 0   | **v0.9 prereqs:** UX-spec P0-0 (per-cluster backend) + P0-1 (list crash / namespace filter); P0-2 + P0-3 land here or alongside step 2a                      |
| 1   | Project CRD + controller (single-label membership; webhook reserves `airunway.ai/agent-runtime`, `role=agent`; unblocks UI)                                  |
| 2a  | Projects list / Resources tab / wizard + "Add to project" in core                                                                                            |
| 2b  | _(stretch)_ Topology tab (react-flow, caps, fallback) — consumes the step 3 `useMetrics` hook                                                                |
| 3a  | Observability tab — app-metrics panel via ring-buffer path; vLLM / Ray Serve / TEI engine adapters                                                           |
| 3b  | Prometheus-source path: new `prometheus.in-cluster` probe, Prom-query client in backend, source-selector behind `useMetrics` (ring buffer stays as fallback) |
| 3c  | IG diagnostics panel (gRPC `GadgetManager.RunGadget`) + IG smoke CI                                                                                          |
| 4   | AKS overlay scaffold + `useAksContext` (probes in shared §7.1 registry, incl. `aks.dcgm-exporter`) + Azure tab placeholder                                   |
| 5   | KAITO preset step; WI / ACR / HTTPRoute / AOAI-credential-helper glue; Portal deep-link drawer                                                               |
| 6   | AKS IG enrichers; GPU telemetry panel via DCGM-over-Prom (when both detected); TTFT-distribution histogram (when Prom present); polish; docs; E2E checklist  |

Steps 1 → 2a → 3a → 3b are the critical path. Step 2b can be cut at
any point if v1.0 is running hot; step 3c and steps 4–6 can fork off
in parallel after step 3a if multiple engineers are available. Step 0
gates step 1 — no Project CRD work before P0-0 and P0-1 are merged.
The conditional-on-Prom renderings in step 6 (GPU telemetry,
TTFT-distribution histogram) degrade gracefully when Prometheus is
absent, so step 3b's discovery edge cases do not block step 6 from
shipping.
...
> **Status of these targets (v1.0):** _informational, not gated._ The
...
| Plugin bundle size            | Core plugin **≤ 1.5 MB** gzipped (includes react-flow ~80 KB gz for topology); AKS overlay **≤ 750 KB** gzipped. _Informational in v1.0; CI gating deferred to v1.1._ |
| Project-detail TTI            | **≤ 2.5 s** on a cluster with 50 Projects / 500 labeled resources, on a warm Headlamp session. _Informational in v1.0; benchmark harness deferred to v1.1._           |
| Topology tab first-paint      | **≤ 2 s** at 50 nodes; falls back to Resources tab beyond the 100-node cap (§6a). _Informational in v1.0._                                                            |
| Observability tab first-paint | **≤ 1.5 s** to render the Project-specific overview strip; app-metrics and IG panels stream in independently. _Informational in v1.0._                                |
| Detection latency             | All §7.1 registry probes complete in **≤ 1 s** or fail open. _Informational in v1.0._                                                                                 |
...
- **Validating webhook on `Project.spec.selector` (narrow).** v1.0
  ...
  - empty `matchLabels` _and_ empty `matchExpressions` (no selector
  ...
  cases that have no legitimate use. **Forward-compat:** the
  webhook must not reject selectors that reference the reserved
  agent-runtime keys `airunway.ai/agent-runtime` or
  `airunway.ai/role=agent` (per §7.1 reservation note) — those keys
  are unused in v1.0 but activate in v1.1, and v1.0 webhook
  behaviour must not paint that future work into a corner.
...
1. **KAITO preset table.** _Resolved._ v1.0 ships
...
2. **Sidebar placement.** _Resolved._ Projects sits **under the existing
...
4. **Upstream infra-metadata gaps — demo-to-real catalog.** With
   the cloud-neutral rule (§3/§4) holding firm, several facts a
   developer-demo viewer would expect to see come from places this
   plugin cannot reach directly. The Topology demo at
   `plugins/examples/runway/` exposes the largest concentration of
   these gaps in one screen, so it doubles as the running checklist.
   Grouped by destination:

   **(a) AI Runway controller / CRD work** (Runway maintainers)

   - **AOAI routing/fallback as a first-class `ModelDeployment`
     concept.** v1.0 ships a credential helper (§9.6) but no
     routing. Unlocks: the demo's "routing decisions · last 15m"
     KV section and the AOAI-fallback edge.
   - **`Project.spec.references` field** (currently reserved name
     only, §5) for cross-Project shared resources. Unlocks: the
     "shared embedding model" pattern called out in §5.
   - **Agents as a first-class concept** — see §14.7.
   - **`calls-runtime` edge support** — see §14.8.

   **(b) KAITO CR-status / preset metadata** (KAITO maintainers)

   - More families exposed in `kaitollmconfig.json` (today: falcon,
     mistral, phi-2, phi-3, qwen, llama3 — no embeddings family).
     Unlocks: an embeddings preset for the demo's `bge-embeddings`
     node.
   - In-cluster KAITO version surfaced more cleanly (today the
     overlay reads CRDs + an annotation). Unlocks: tighter
     preset-version-mismatch warnings.

   **(c) IG gadget enrichers** (this plugin)

   - Per-Pod aggregation of `trace_tcpretrans` → NCCL bandwidth
     gauge. Unlocks: the demo's NCCL B/W bar in the GPU block.
   - Per-Pod aggregation of `top_blockio` → PVC read MB/s. Unlocks:
     the demo's PVC throughput rows.
   - Both ride on the existing v1.0 curated gadget set (§8) — no
     IG-side work, just panel-side enrichers; tracked as v1.1.

   **(d) New exporters / data sources needed**

   - **DCGM exporter** for GPU hardware-counter telemetry. v1.0
     ships `aks.dcgm-exporter` detection + renders the panel when
     Prom is also present (§9 GPU telemetry panel). Cluster admins
     installing DCGM is out of plugin scope; the §9 CTA explains.
   - **Billing exporter** for $/1M tokens, GPU-hours, cost vs.
     AOAI. No standard exists. Unlocks: the demo's cost rows in
     model drawers and the project-header cost stat. v2.0 at
     earliest.
   - **Vector-DB metrics adapter** (qdrant `/metrics`, etc.) for
     search QPS, recall, vector count. v2.0 alongside vector-DB-
     shaped Projects.

   **(e) New engine adapters** (this plugin)

   - Per-engine adapter coverage drives §6 panel completeness. v1.0
     ships vLLM, Ray Serve, TEI. Triton, SGLang, TRT-LLM,
     TorchServe, Ollama, ONNX Runtime are on the "contribute one →"
     CTA path; in-tree as v1.1 demand surfaces.

   Owner: KAITO maintainers + AI Runway maintainers + this plugin.
   Track as a living list; do not block v1.0.
...
5. **Topology stretch decision.** Re-evaluate mid-v1.0: is the rest of
   ...
   starts. **Visual reference for either outcome:**
   `plugins/examples/runway/` already shows what the shipped view
   would look like — the hard call is whether to wire the data path,
   not whether the visual answer is right.
...
7. **Agents as a first-class concept.** The Topology demo
   (`plugins/examples/runway/`) puts an Agent lane between Ingress
   and Models with agent-shaped face/drawer metrics (tool calls/s,
   turns/conversation, routing-decision breakdown, semantic cache
   hit). The v1.0 spec does not detect agent runtimes; the demo's
   forward-looking README section names this gap. v1.1 work, scoped
   sketch:

   - §7 framework table grows rows for `langgraph`, `llamaindex`,
     `autogen`, `smolagents` (and the homegrown case).
   - Override label `airunway.ai/agent-runtime=<name>` paralleling
     `airunway.ai/engine=<name>` (§7.1). **Key is reserved in
     v1.0**; webhook (§13) must accept it.
   - Role label `airunway.ai/role=agent` paralleling
     `role=prefill|decode` (§4 controller addendum) — topology
     renderer uses it for lane assignment without learning what
     kind the Pod is. **Key is reserved in v1.0.**
   - New `AgentRuntimeAdapter` registry parallel to `EngineAdapter`
     (§6) covering tool calls/s, routing decisions, turns/session,
     semantic cache hit. One adapter file per runtime.

   Owner: spec lead; decide before v1.1 scoping locks. Source:
   `docs/superpowers/specs/projects-and-aks-overlay-feedback.md` §1.

8. **`calls-runtime` edge kind.** The Topology demo draws agent →
   model edges using a placeholder kind not in the §6a edge table,
   which today only covers K8s-state-derivable relationships.
   Without a runtime-call edge, an agent-shaped Project's topology
   shows the agent and its downstream models as disconnected
   islands. v1.1 work, separate from §14.7 because the edge kind
   has value beyond agents (any Service-to-Service runtime call).
   Leaning answer: **declarative-first** — Agent CRD field
   (`spec.calls: [Service refs]`) or env-var convention
   (`AIRUNWAY_CALLS_<NAME>=http://svc`), authoritative when set;
   **IG-observed fallback** via `trace_tcp` over a configurable
   window when no declaration exists. The §7.1 detection contract
   (`{ detected, confidence, source }`) maps cleanly. No new v1.0
   IG client work — `trace_tcp` is already in the curated 9-gadget
   set (§8).
...
| IG's GPU coverage is allocation- and runtime-event shaped (stack-attributed CUDA allocs via `profile_cuda` / `top_cuda_memory`, NCCL-adjacent network signals via `profile_tcprtt`, `trace_tcp`, etc.); hardware-counter metrics (utilization %, SM occupancy, temp, power) are not in IG's model and won't be | Low        | Low — known limit, clearly attributed                                            | The IG diagnostics panel surfaces what IG actually does well (allocation attribution, OOM root cause, inter-replica network behavior) and labels these as such. Hardware-counter metrics in v1.0 come from **DCGM via Prometheus** when both probes (`aks.dcgm-exporter` + `prometheus.in-cluster`) detect — see §9 GPU telemetry panel. Clusters without DCGM or without Prom see a placeholder CTA and use their existing DCGM / Grafana stack outside the plugin                                   |
| Demo expectations (e.g., the topology demo at `plugins/examples/runway/`) outrun what v1.0 actually ships — viewer sees a panel and assumes it works on any cluster                                                                                                                                            | Medium     | Low — credibility cost, not user-blocking                                        | Every demo-vs-shipped gap is enumerated in §14.4 grouped by destination (Runway controller / KAITO / IG enrichers / new exporters / new engine adapters); the demo README at `plugins/examples/runway/README.md` re-tiers its "forward-looking" section into **In v1.0 (subject to detection)** vs **In v1.1+** so a stakeholder viewing the demo sees what's wired today vs. what isn't; conditional-on-Prom / conditional-on-DCGM panels show their detection CTAs honestly when the dep is missing |
...
| AKS detection false positives on non-AKS clusters using the same node label out of habit                                                                                                                                                                                                                       | Low        | Medium — overlay activates wrongly                                               | Detection requires _both_ the cluster-wide label and at least one Azure-specific CRD/identity signal before activating                                                                                                                                                                                                                                                                                                                                                                                |
